##########################################################################
# Capture + display + store raw cubes to HDF5 (cv2Capture)
##########################################################################
...
def main() -> None:
    ...
    flatfield = np.asarray((2**8) * np.random.random((height, width)), dtype=np.uint16)
    inten = np.zeros(depth, dtype=np.uint16)
    data_cube_corr = np.zeros((depth, height, width), dtype=np.uint16)

    logging.basicConfig(level=logging.DEBUG)
    logger = logging.getLogger('cv2_capture_savehdf5_display')
    ...
    try:
        while not stop:
            ...
            if frame_idx >= depth:
                ...
                # Background + flatfield correction (computed but not saved here)
            ...
            if (current_time - last_stats) >= measure_time:
                logger.debug('Status:cubes generated/s: %s', num_cubes_generated / measure_time)
                logger.debug('Status:cubes queued/s: %s', num_cubes_stored / measure_time)
            ...
            if (current_time - last_display) >= display_interval:
                ...
                # cv2Capture exposes measured_fps; other backends may not.
